...
func TestCmdWithOTELSpanID(t *testing.T) {
	// Skip if tart is not available
	if !Installed() {
		t.Skip("tart command not found")
	}
	...
	// Mock the tart command by creating a simple script that prints environment variables
	// We'll use a command that doesn't actually invoke tart but allows us to test env passing
	originalPath := os.Getenv("PATH")
	
	// Create a temporary directory for our mock tart command
	tempDir := t.TempDir()
	mockTartPath := tempDir + "/tart"
	
	// Create a mock tart script that prints the OTEL_PARENT_SPAN_ID environment variable
	mockScript := `#!/bin/bash
echo "OTEL_PARENT_SPAN_ID=${OTEL_PARENT_SPAN_ID}"
`
	err := os.WriteFile(mockTartPath, []byte(mockScript), 0755)
	require.NoError(t, err)
	
	// Temporarily modify PATH to use our mock tart
