...
func TestGHACacheV2(t *testing.T) {
	...
	httpCacheURL := "http://" + http_cache.Start(ctx, http_cache.WithAzureBlobOpts(azureblob.WithUnexpectedEOFReader()))
...
func TestGHACacheV2UploadStream(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			httpCacheURL := "http://" + http_cache.Start(t.Context(), http_cache.WithAzureBlobOpts(azureblob.WithUnexpectedEOFReader()))
