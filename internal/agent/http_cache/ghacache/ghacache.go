...
func New(cacheHost string, storage blobstorage.BlobStorageBacked) *GHACache {
	cache := &GHACache{
		cacheHost:   cacheHost,
...
func (cache *GHACache) get(writer http.ResponseWriter, request *http.Request) {
	...
	info, err := cache.storage.Info(request.Context(), keysWithVersions[0], keysWithVersions[1:])
	if err != nil {
...
func (cache *GHACache) reserveUploadable(writer http.ResponseWriter, request *http.Request) {
	...
	uploadID, err := cache.storage.MultipartUploadCreate(request.Context(), httpCacheKey(jsonReq.Key, jsonReq.Version))
	if err != nil {
...
func (cache *GHACache) updateUploadable(writer http.ResponseWriter, request *http.Request) {
	...
	urlInfo, err := cache.storage.MultipartUploadPartURL(request.Context(),
	...
	for header, value := range urlInfo.ExtraHeaders {
		uploadPartRequest.Header.Set(header, value)
	}
...
func (cache *GHACache) commitUploadable(writer http.ResponseWriter, request *http.Request) {
	...
	err = cache.storage.MultipartUploadCommit(request.Context(),
		httpCacheKey(uploadable.Key(), uploadable.Version()),
		uploadable.UploadID(),
		parts,
	)
