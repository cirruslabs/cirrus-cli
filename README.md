...
By default, Cirrus CLI uses [the default folder for user-specific cached data](https://golang.org/pkg/os/#UserCacheDir)
for storing blob artifacts produced by [Cache Instruction](https://cirrus-ci.org/guide/writing-tasks/#cache-instruction).
[Similar to Cirrus CI](https://cirrus-ci.org/guide/writing-tasks/#http-cache) the CLI can use a caching HTTP server for
more efficient sharing of cached artifacts between tasks executed on different physical hosts.
